...
func (s *Server) verifyBallotCommit(cert *LogCert) {
    ...
    for _, args := range argsQ {
        ...
        if cert.view != msg.View {
            zlog.Warn("verify ballot, CommitMsg error, view(%d) != msg.View(%d)", cert.view, msg.View)
            continue
        }
        ...
        if ok := Verify(Digest(msg), args.Sign, s.peers[args.Msg.PeerId].pubkey); !ok {
            zlog.Warn("verify ballot, CommitMsg verify error, seq: %d, from: %d", msg.Seq, msg.PeerId)
            continue
        }
    ...
    if cert.getStage() == CommitStage && cert.commitBallot() >= 2*s.f+1 {
