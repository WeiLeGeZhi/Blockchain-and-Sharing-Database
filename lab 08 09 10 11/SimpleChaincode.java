...
public class SimpleChaincode extends ChaincodeBase {
    ...
    private Response invoke(ChaincodeStub stub, List<String> args) {
        ...
        try {
            transferAmount = Integer.parseInt(args.get(2));
        } catch (NumberFormatException e) {
            return newErrorResponse("Invalid transaction amount, expecting a integer value");
        }

        String accountFromValueStr = stub.getStringState(accountFromKey);
        if (accountFromValueStr == null) {
            return newErrorResponse("Entity not found: " + accountFromKey);
        }

        String accountToValueStr = stub.getStringState(accountToKey);
        if (accountToValueStr == null) {
            return newErrorResponse("Entity not found: " + accountToKey);
        }

        int accountFromValue = Integer.parseInt(accountFromValueStr);
        int accountToValue = Integer.parseInt(accountToValueStr);

        accountFromValue -= transferAmount;
        accountToValue += transferAmount;

        _logger.info(String.format("Transfer %d from %s to %s", transferAmount, accountFromKey, accountToKey));
        _logger.info(String.format("New values: %s = %d, %s = %d", accountFromKey, accountFromValue, accountToKey, accountToValue));
    ...
    private Response delete(ChaincodeStub stub, List<String> args) {
        ...
        _logger.info("Deleted state for key: " + key);
    ...
    private Response query(ChaincodeStub stub, List<String> args) {
        ...
        if (val == null) {
            return newErrorResponse(String.format("No value found for key: %s", key));
        }

        String jsonResp = String.format("{\"Name\":\"%s\",\"Amount\":\"%s\"}", key, val);
        _logger.info("Query Response: " + jsonResp);
        // 这里记查询的状态余额为val
